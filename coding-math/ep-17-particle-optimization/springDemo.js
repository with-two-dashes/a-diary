...
registerRenderCycle(({ context, mousePosition }) => {
  ...
  const distanceX = springPoint.x - weight.x
  const distanceY = springPoint.y - weight.y
  const distance = Math.sqrt((distanceX * distanceX) + (distanceY * distanceY))
  const springForce = (distance - springLength) * springStiffnessK
  const accelerationX = (distanceX / distance) * springForce
  const accelerationY = (distanceY / distance) * springForce

  weight.vx += accelerationX
  weight.vy += accelerationY
